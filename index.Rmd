...
```{python, collapse = TRUE, error = TRUE}
print(fibonacci(505), count)
```
]
.panel[.panel-name[Fibo mais robusto, com contador]
```{python, collapse = TRUE}
from timeit import default_timer as timer
...
start = timer()
print(fibonacci(6), count, end - start)
end = timer()
```
]
.panel[.panel-name[Nova tentativa]
```{python, collapse = TRUE, error = TRUE}
print(fibonacci(1000), count)
```
]
]
